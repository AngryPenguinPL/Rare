...
class DownloadsTab(QWidget, Ui_DownloadsTab):
    ...
    def __init__(self, parent=None):
        ...
        start_t = time.time()
        has_updates = False
        for rgame in self.rare_core.updates:
            has_updates = True
            self.add_update(rgame)
        print(f"Update fetching: {time.time() - start_t}")

        self.update_text.setVisible(has_updates)

        self.queue_widget.item_removed.connect(self.queue_item_removed)

        # self.signals.game.install.connect(self.get_install_options)
        for rgame in self.rare_core.games_and_dlcs:
            rgame.signals.game.install.connect(self.get_install_options)
        ...
        self.signals.download.enqueue_game.connect(
            lambda app_name: self.add_update(self.rare_core.get_game(app_name))
        )
    ...
    def add_update(self, rgame: RareGame):
        ...
        print(f"Update widget creation time: {time.time() - start_t}")
    ...
    def statistics(self, app_name: str, ui_update: UIUpdate):
...
class UpdateWidget(QWidget):
    ...
    def update_game(self, auto: bool):
        ...
        self.update_signal.emit(
            InstallOptionsModel(app_name=self.rgame.app_name, silent=auto)
        )  # True if settings
