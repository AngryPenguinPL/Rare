...
from rare.shared.rare_core import RareCore, RareCoreSingleton
...
class LibraryWidgetController(QObject):
    _icon_container: QWidget
    _list_container: QWidget
    _game_utils = None

    def __init__(self, icon_container: QWidget, list_container: QWidget, game_utils, parent: QWidget = None):
        ...
        self.rare_core: RareCore = RareCoreSingleton()
    ...
    def add_game(self, rgame: RareGame, utils):
        return self.add_widgets(rgame, utils)

    def add_widgets(self, rgame: RareGame, utils) -> Tuple[IconGameWidget, ListGameWidget]:
        icon_widget = IconGameWidget(rgame, utils)
        list_widget = ListGameWidget(rgame, utils)
    ...
    def update_list(self, app_names: List[str] = None):
        if not app_names:
            ...
            icon_widgets = self._icon_container.findChildren(IconGameWidget)
            list_widgets = self._list_container.findChildren(ListGameWidget)
            icon_app_names = set([iw.game.app_name for iw in icon_widgets])
            list_app_names = set([lw.game.app_name for lw in list_widgets])
            games = list(self.rare_core.games)
            ...
            for app_name in new_icon_app_names:
                ...
                iw = IconGameWidget(game, self._game_utils)
                self._icon_container.layout().addWidget(iw)
            for app_name in new_list_app_names:
                game = self.rare_core.get_game(app_name)
                lw = ListGameWidget(game, self._game_utils)
