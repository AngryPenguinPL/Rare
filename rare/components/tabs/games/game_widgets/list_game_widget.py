...
class ListGameWidget(GameWidget):
    ...
    def __init__(self, rgame: RareGame, game_utils):
        super(ListGameWidget, self).__init__(rgame, game_utils)
        self.setObjectName(f"{rgame.app_name}")
        self.ui = ListWidget()
        self.ui.setupUi(self)

        self.ui.title_label.setText(f"<h3>{self.game.app_title}</h3>")
        ...
        self.ui.launch_btn.setText(
            self.tr("Launch") if self.igame is not None else self.tr("Link/Play")
        )
        ...
        if self.igame:
            self.ui.version_text.setText(self.igame.version)
