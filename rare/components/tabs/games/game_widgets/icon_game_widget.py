...
class IconGameWidget(GameWidget):
    ...
    def __init__(self, rgame: RareGame, game_utils):
        super(IconGameWidget, self).__init__(rgame, game_utils)
        self.setObjectName(f"{rgame.app_name}")
        self.setFixedSize(ImageSize.Display)
        self.ui = IconWidget()
        self.ui.setupUi(self)

        game_utils.finished.connect(self.game_finished)

        self.ui.title_label.setText(f"<h4>{self.game.app_title}</h4>")
        ...
        game_utils.game_launched.connect(self.game_started)
