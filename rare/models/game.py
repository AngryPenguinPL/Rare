...
class RareGame(QObject):
    ...
    def __init__(self, game: Game):
        ...
        self.owned_dlcs: List[RareGame] = list()
    ...
    def repair(self):
        ...
        if not os.path.exists(repair_file):
            QMessageBox.warning(
                None,
                self.tr("Warning - {}").format(self.igame.title),
                self.tr(
                    "Repair file does not exist or game does not need a repair. "
                    "Please verify the game first"
                ),
            )
            return
        self.signals.game.install.emit(
    ...
    def uninstall(self, utils):
        accepted = utils.uninstall_game(self.game.app_name)
        if accepted:
            self.signals.game.uninstalled.emit(self.game.app_name)
            self.igame = None
            self.set_pixmap()

    __verify_worker: Optional[VerificationWorker] = None
    __verify_worker_connection: Optional[QMetaObject.Connection] = None

    def verify(self) -> None:
        worker = VerificationWorker(self.game.app_name)
        worker.signals.started.connect(self.start_progress)
        worker.signals.status.connect(
            lambda num, total, app: self.update_progress(num * 100 // total)
        )
        worker.signals.summary.connect(
            lambda fail, miss, app: self.finish_progress(fail, miss, app)
        )
        self.__verify_worker = worker

    def verify_worker(self) -> Optional[VerificationWorker]:
        return self.__verify_worker
